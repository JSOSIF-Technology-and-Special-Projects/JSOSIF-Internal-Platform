...
const credentials = JSON.parse(fs.readFileSync(path.resolve(GOOGLE_ANALYTICS_CREDS), "utf-8"));
...
export async function GET(): Promise<NextResponse<AnalyticsResponse | { error: string }>> {
  try {
    ...
    const metricsData = response.rows[0].metricValues.map((value) => parseFloat(value.value || '') || 0);
